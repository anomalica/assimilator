...
--
-- The workbench opens knowledge.db READ-ONLY by design, and that is right: a
-- name written straight into the database does not survive a rebuild, because
-- the graph is re-imported from the digests and only the curation ledger is
-- replayed. A rename must go through rename_node, which writes the ledger entry
-- replay re-applies. So the reviewer PROPOSES here and the assimilator APPLIES.
--
