...
@click.pass_context
def apply_renames_cmd(ctx: click.Context, dry_run: bool) -> None:
    """Apply node renames proposed by a reviewer.

    The workbench opens the graph read-only by design, and a name written
    straight into the database would not survive a rebuild - the graph is
    re-imported from the digests and only the curation ledger is replayed. So a
    proposal is applied HERE, through rename_node, which writes the ledger entry
    that replay re-applies.

    A proposal whose node no longer resolves is marked LOST, not dropped: a
    reviewer who asked for a change is owed an answer either way.
    """
    ...
    for p in proposals:
        ...
        if row is None:
            ...
            continue
        clash = conn.execute(
            "SELECT 1 FROM nodes WHERE name = ? AND retired_at IS NULL AND id <> ?",
            (p["proposed_name"], row[0]),
        ).fetchone()
        if clash:
            click.echo(
