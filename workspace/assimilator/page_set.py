...
at least MIN_SOURCES positively established work roots. This cuts the long tail of thin /
junk entities (most nodes have <=2 claims) without an AI scorer.

This work-spread floor is not claim independence. Exact evidence-unit overlap and
assertion-origin lineage remain separate and are applied by the evidence model.
...
def page_set_node_ids(
    ...
    A node's claims are those it speaks OR is referenced in; unknown work roots
    add zero and Record identity is never a fallback. Retired (merged-away) nodes
    are excluded - the survivor carries the consolidated count.
    """
    fc, fs = floor()
    ...
    rows = conn.execute(
        ...
        GROUP BY n.id
        HAVING COUNT(DISTINCT x.cid) >= ? AND COUNT(DISTINCT pr.id) >= ?
        ORDER BY n.id
        """,
        (mc, ms),
    ).fetchall()
