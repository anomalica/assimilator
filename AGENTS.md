...
- The canonical pipeline architecture and contracts live in `/home/mark/repos/anomalica/anomalica/architecture/`. Check them and current consumers before changing graph or brief interfaces.
- Application code is in `workspace/assimilator/`; tests are in `workspace/tests/`.
- Shared digest models, transports, embedding clients and shared-tree Git helpers belong in `/home/mark/repos/anomalica/anomalica-common/`, not in local copies.
...
- Publishing writes to the shared `/home/mark/repos/anomalica/content/` working tree. Commit only explicit pathspecs on the expected branch through `anomalica_common.shared_tree.commit_paths`; preserve other sessions' staged and unstaged work.
...
- Run focused host tests from `workspace/` with `PYTHONPATH=.:/home/mark/repos/anomalica/anomalica-common/src` when that is faster.
